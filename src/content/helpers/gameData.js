...
function _injectBridge() {
  ...
  script.textContent = `(function() {
    ...
    function enrichCurrentCity(cities, selectedCityId, model) {
      ...
      if (res) {
        city.resources = normalizeResources(res) || city.resources;
      }
        ...
        window.postMessage({
          ...
          payload: {
            ...
            debug: {
              ...
              hasSelectedBuildings: parsed.cities.some(function(city) { return city.id === parsed.selectedCityId && !!city.buildings; }),
              hasMilitary: parsed.cities.some(function(city) { return !!city.military; })
            }
          }
        }, '*');
      } catch(e) {}
...
const gameData = {
  ...
  // Thông tin debug tối giản để kiểm tra bridge trên trang game thật.
  getDebug() {
